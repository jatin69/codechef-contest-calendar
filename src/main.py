...
# import user modules
import authenticate_user, authenticate_service_account
...
def makeEventsonGoogleCalendar(events):
    ...
    for e in events:
        uniqueContestId =  get_unique_contest_id.getUniqueIdForContest(e['contestCode'])
        currentEventBody = {
        ...
        try:
            ...
            currentEvent = service.events().insert(calendarId=config.codechefCalendarId, body=currentEventBody).execute()
            ...
            if err.resp.status in [409]:
                ...
                if currentEvent['start']['dateTime'] == currentEventBody['start']['dateTime'] and \
                    ...
                    print("Already Added - {0}".format(currentEvent['summary']))
                else:
